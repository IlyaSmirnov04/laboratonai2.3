class Book:
    ...
    def __str__(self):
        return f"Книга {self._name}. Автор {self._author}" # метод перегружен

    def __repr__(self):
        return f"{self.__class__.__name__}(name={self._name!r}, author={self._author!r})" # метод может быть унаследован

    @property
    def name_autor(self):
        return self._name, self._author
...
class PaperBook(Book):
    ...
    def name1_autor(self):
        return self._name, self._author,self.pages
...
class AudioBook(Book):
    ...
    def __str__(self):
        return f"Аудиокнига {self._name}. Автор {self._author}. Время {self.duration}"

    @property
    def name2_autor(self):
        return self._name, self._author, self.duration
